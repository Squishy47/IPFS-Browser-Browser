...
const mapStateToProps = (state) => {
	const filteredList = state.fileListSublist
		.filter((file) => file.type === fileTypes.directory)
		.filter((file) =>
			state.path.join("").trim() === state.pathSublist.join("").trim()
				? !state.selectedFiles.find((f) => f.name === file.name)
				: true
		);
	return {
