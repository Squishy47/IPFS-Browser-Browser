...
class File extends Component {
	render() {
		...
		const realSize =
			typeof size !== "undefined" && type !== fileTypes.directory
				? getHumanFileSize(size)
				: null;
		return (
			<div
				className="File"
...
const mapDispatchToProps = (dispatch, ownProps) => {
	return {
		...
		handleDoubleClick: (event) => {
			if (ownProps.type === fileTypes.file) {
				if (
					config.isEditableFilePattern.test(ownProps.name) ||
					ownProps.editable
				) {
