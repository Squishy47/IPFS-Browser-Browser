...
export const refreshFileList = () => (dispatch, getState) => {
	...
	APIHandler.getFileList(path.join("/"))
		.then((r) => {
			dispatch(setLoading(false));
...
export const refreshFileListSublist = () => (dispatch, getState) => {
	...
	APIHandler.getFileList(pathSublist.join("/"))
		.then((r) => {
...
export const getFileContent = (fileName) => (dispatch, getState) => {
	...
	APIHandler.getFileBody(path.join("/"), fileName)
		.then((blob) => {
...
export const renameItem = (fileName, newFileName) => (dispatch, getState) => {
	const { path } = getState();
	dispatch(setLoading(true));
	APIHandler.renameItem(path.join("/"), fileName, newFileName)
		.then((blob) => {
...
export const createNewFolder = (createFolderName) => (dispatch, getState) => {
	...
	APIHandler.createFolder(path.join("/"), createFolderName)
		.then((r) => {
...
export const removeItems = (files) => (dispatch, getState) => {
	...
	dispatch(setLoading(true));
	APIHandler.removeItems(path.join("/"), filenames)
		.then((r) => {
			dispatch(setLoading(false));
			dispatch(refreshFileList());
		})
...
export const moveItems = (files) => (dispatch, getState) => {
	...
	const destination =
		pathSublist.join("/") + "/" + selectedFolderSublist.name;
	const filenames = files.map((f) => f.name);

	dispatch(setLoading(true));
	APIHandler.moveItems(path.join("/"), destination, filenames)
		.then((r) => {
			dispatch(setLoading(false));
...
export const copyItems = (files) => (dispatch, getState) => {
	...
	dispatch(setLoading(true));
	APIHandler.copyItems(path.join("/"), destination, filenames)
		.then((r) => {
			dispatch(setLoading(false));
